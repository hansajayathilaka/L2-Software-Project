...
class NFTRequestForm(forms.Form):
    ...
    price = forms.DecimalField(label="Estimation Price (MATIC)", required=False)
    thumbnail = forms.ImageField(label='Thumbnail', required=False)
    attachments = forms.FileField(label='Attachments', widget=forms.ClearableFileInput(attrs={'multiple': True}), required=False)
